...
const BLOCK_DROPDOWN = [0, 0, 0, 1, 1]; // 0 => false, 1 => true
...
export class LevelManagement {
    ...
    static createLevel() {
        ...
        for (let i = 0; i <= NUMBER_OF_LEVEL; i++) {
            listLevel.set(i+1, new Level(LIST_POINT_PER_LEVEL[i], BLOCK_RATE_PER_LEVEL[i], FOOD_TIMING_PER_LEVEL[i], BLOCK_DROPDOWN[i]));
        }
